...
- Jellyfin: Movies not merged by initial scan can be merged thanks to "Merge Version" Jellyfin plugin. Shows are not merged because in this case it cause troubles (like whole season merged into one media item).
- 3 Jellyfin plugins are pre-installed, including "subbuzz" which is not enabled on library scan but can be used on induvidual items. You can enable it on library scan if you want but beware the added network calls to Real-Debrid it will do.
