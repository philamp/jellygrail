...
VERSION = "v1.4.1" # Should be aligned to settings.env.template and early_init.sh and kodi addon init_context!!!
...
class genericClass:
	...
	def setVerboseLog(cls, enabled: bool):
		cls.verboseLog = bool(enabled)

	@classmethod
	def isVerboseLog(cls):
		return cls.verboseLog
...
SIMPLE_SCAN_MODE = True if (os.getenv('SIMPLE_SCAN') or "n") == "y" else False
...
WEBDAV_REMOTE_HOST_PORT = LAN_IP + ":" + str(WEBDAV_REMOTE_INTERNAL_PORT)
...
LITPREFLANG = LANGDICT[PREFLANG]['english'][0]
