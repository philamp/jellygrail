...
class sqlKodiDB:
    ...
    def fetch_media_str_with_id(self, idtofetch, tabletofetch, columntofetch):
        with self as cursor:
            cursor.execute(f"SELECT ttf.c00, ttf.strFileName, ttf.strPath, ttf.uniqueid_value, ttf.uniqueid_type FROM {tabletofetch}_view ttf WHERE {columntofetch} = %s", (idtofetch,))
            return cursor.fetchall()
